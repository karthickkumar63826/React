...
function Users() {
  ...
  let [filteredSearch, setFilteredSearch] = useState(users);
  const handleSearch = (search) => {
    if (search === "" || search === undefined) {
      setFilteredSearch(users);
    }
    else {
      
      const filteredUsers = filteredSearch.filter(
        (u) =>
          u.name.firstname.includes(search) || u.name.lastname.includes(search) || u.id === Number(search)
      );
      setFilteredSearch(filteredUsers);
    }
  };

  return (
    <>
      <UsersTable users={filteredSearch} onSearch={handleSearch}></UsersTable>
    </>
  );
}
